let
    Lenovo-P1 = "ssh-rsa ";
    JuiceSSH = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEcP5JDW+JKSD04YGd+giu8oGCVGKjh7ZSap0UbNUYhP JuiceSSH";
    system1 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPkmhGLFvUYG/rvZ12j+mamfEcXAh8xeMfYF3B4VBnVt";
    mycloud-nixos = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINW3O4vGdlI7ZKhqTcuo4rFb97W3B9oquKMxoZI/ijkw root@mycloud-nixos";
    mycloud-nixos-2 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBTIYFWBPT6gv2Udl37nRUPULH9oBDgg5q4zZOB8vWCA root@mycloud-nixos-2";
in {
  "nextcloud_admin_pass.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "hetzner_private_key.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "backblaze_env.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "backblaze_password.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "backblaze_repo.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "mlflow-artifacts-key.age".publicKeys = [ Lenovo-P1 JuiceSSH system1 mycloud-nixos mycloud-nixos-2 ];
  "paperless-admin-password.age".publicKeys = [ Lenovo-P1 system1 mycloud-nixos mycloud-nixos-2 ];
}
